...
public sealed partial class Portal
{
    ...
    public async Task DecideSolidarity(Guid id, JsonObject body, string actor)
    {
        ...
        if (decision != previous || note != r["public_note"]?.ToString()) {
            await Event(c, id, "solidarity_" + decision, note);
            await using var policyCmd = Database.Command(c, "SELECT data->>'emailPolicy' FROM andrade_portal.settings WHERE id=true");
            if(decision != previous || (await policyCmd.ExecuteScalarAsync())?.ToString() == "all") await email.Queue(c, id, "solidarity_" + decision);
        }
